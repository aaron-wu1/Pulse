...
pub fn get_system_memory_stats() -> io::Result<SystemMemoryStats> {
    ...
    let mut wired: u64 = 0;
    let mut memsize: u128 = 0;

    let mut size = std::mem::size_of::<u64> as size_t;
    let mut size1 = std::mem::size_of::<u128> as size_t;
    ...
    unsafe {
        ...
        if sysctlbyname(
            memsize_key.as_ptr(),
            &mut memsize as *mut u128 as *mut c_void,
            &mut size1,
            std::ptr::null_mut(),
            0,
        ) != 0
    ...
    //     wired: wired * 4096,
    //     memsize: bytes_to_gb(memsize as u64 * 16384) as u128,
    // })
